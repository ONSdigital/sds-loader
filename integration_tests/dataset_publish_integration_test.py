...
class DatasetPublishIntegrationTest(unittest.TestCase):
    ...
    def setup_class(cls):
        ...
        cls.http_service = HttpService.create(True)
    ...
    def test_dataset_publish_success(self):
        ...
        * We call SDS metadata endpoint to check if the dataset is published with the correct survey ID.
        ...
        response = self.http_service.make_get_request(self.dataset_create_endpoint)
        ...
        response = self.http_service.make_get_request(f"{CONFIG.SDS_URL}/v1/dataset_metadata?survey_id=test_survey_id_dataset_create_test&period_id=test_period_id")

        self.assertEqual(len(response.json()), 1, "Expected one dataset metadata entry in the response.")
