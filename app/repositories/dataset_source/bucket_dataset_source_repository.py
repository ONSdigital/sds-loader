...
class BucketReader(Protocol):
    ...
    def get_blobs(self,
                  ...
                  directory: Optional[str] = None) -> list[Blob]:
        ...
...
class BucketDatasetSourceRepository(DatasetSourceRepositoryInterface):
    ...
    def get_oldest_filename(self) -> str | None:
        ...
        valid_blobs = (
            blob
            for blob in blobs
            if blob.updated is not None and blob.name is not None
        )
    ...
    def get_raw_data(self, file_name: str) -> RawDataset | None:
        ...
        data_bytes = self._bucket_reader.read(
            file_name,
            bucket_name=self._settings.dataset_bucket_name
        )
